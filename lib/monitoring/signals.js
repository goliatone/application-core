...
var DEFAULTS = {
    autoinitialize: true,
    signals: ['SIGINT', 'SIGTERM', 'SIGQUIT', 'SIGBREAK', 'uncaughtException'],
    handlers: [],
    timeout: 1 * 1000,
    // timeout: 10 * 1000,
    defaultExitCode: 0,
...
Serene.prototype.upon = function(signals){
    ...
    function once(signal, codeOrError){
        ...
        try {
            self._onShudtdown({signal:signal, data: codeOrError});
        } catch(e) {
            debug(e);
            self.exit(1);
        }
    ...
    signals.map(function(signal){
        debug('processing shutdown handler for %s', signal);
...
Serene.prototype._onShudtdown = function(event){
    ...
    if(event.data instanceof Error){
        ...
        event.data.stack && this.logger.error(event.data.stack);
    }
