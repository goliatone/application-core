...
function createHook(emitter, config){
    return function(hook, ...args){
        ...
        return Promise.reduce(emitter.listeners(config.pre(hook)), _reducer(), args).then((nargs)=>{
            if(!Array.isArray(nargs)) nargs = [nargs];
            return Promise.all(emitter.listeners(hook).map((fn)=>{
            ...
            return Promise.reduce(emitter.listeners(config.post(hook)), _reducer(), results).then((res)=>{
                if(!Array.isArray(res)) res = [res];
                emitter.emit(config.done(hook), ...res);
                return res;
            });
        });
    };
...
function chainEvents(emitter){
    ...
    function promisifyEvents(eventsResolve, eventsReject){
    ...
    return promisifyEvents.bind(emitter);
}
