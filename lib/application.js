...
class Application extends EventEmitter {
    ...
    loadModuleCommands(modulePathOrId) {
        ...
        this._logger.info('load module commands: %s', dir);
    ...
    _loadModuleExposedCommands() {
        ...
        if (!this.autoloadModulesCommands) {
            this._logger.info('Ignoring sub comamnds, you need to set config option');
            this._logger.info('autoloadModulesCommands in config/app.js to true');
            return Promise.resolve();
        }
        ...
        this._commands.forEach((moduleid) => {
            this._logger.info('Loading subcommand for %s', moduleid);
    ...
    command(eventType, handler, unique = false) {
        ...
        this.on(eventType, (e = {}) => {
            ...
            if (out && typeof e.respondTo === 'function') {
                out.then(res => {
    ...
    wrapRespondTo(event) {
        ...
        event.respondTo = function $responseHandler() {
            ...
            event.respondTo.apply(null, args);
        };
    }
