...
class Application extends EventEmitter {
    ...
    register(instance, name){
        ...
        if(this.hasOwnProperty(name)){
            this.logger.warn('register: we are going to override plugin %s', name);
        }

        function _register(context, name, instance){
            context[name] = instance;
            //move to dispatcher? context.dispatcher.join(name, context.registerReadyEvent).
        ...
        if(typeof out === Promise){
            ...
            }).catch(this.onErrorHandler.bind(this, 'register: Error registering module' + name));
    ...
    handle(event, handler){
        ...
        this.logger.debug('- register handler for %s', event);
