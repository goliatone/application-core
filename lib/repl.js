...
module.exports.init = function(context, config) {
    ...
    const enabled = context.config.get('repl.enabled', false);
    ...
    context.once('run.post', _ => {
        _logger.debug('Module "repl" ready...');
        repl.listen();

        /**
         * We can expose functions in the repl context
         * using our configuration file.
         */
        if (config.context) {
            Object.keys(config.context).map(key => {
                repl.context[key] = config.context[key];
            });
        }
        ...
        //TODO: Expose a method so we dont have to access `repl.context`
