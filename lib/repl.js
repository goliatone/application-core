...
module.exports.init = function(context, config){
    ...
    context.on('run.post', () => {
        ...
        _logger.info('You can listen the repl at "%s:%s".', repl.config.host, repl.config.port);

        repl.on('error', (err)=>{
            _logger.error('Error starting REPL instance...');
            _logger.error('Error:', err.message);
            this.handleModuleError('repl', err);
        });
